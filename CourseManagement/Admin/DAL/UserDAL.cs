    ...
    public class UserDAL
    {
        ...
        public User CheckLogin(string username, string password)
        {
            ...
            using (dbConnection)
            {
                dbConnection.Open();
                var selectQuery =
                    "SELECT `users`.`uid`, `users`.password, `roles`.role_name " +
                    "FROM `users`, `user_has_role`, `roles` " +
                    "WHERE `users`.`uid` = `user_has_role`.user_uid " +
                    "AND `user_has_role`.`roles_role_id` = `roles`.role_id AND users.uid = @username";
